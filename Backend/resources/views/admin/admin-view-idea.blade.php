      ...
      <div class="container">
        <div class="d-flex align-items-center justify-content-between">
          ...
          <div class="d-flex align-items-center">
            ...
            <a href="{{ route('admin.admin-accept-idea', ['id' => $idea->id]) }}"
              class="btn btn-custom px-4 py-2 d-flex align-items-center justify-content-center"><i
                class="fas fa-check mr-2"></i>Accept Idea</a>
            <a href="{{ route('admin.admin-reject-idea', ['id' => $idea->id]) }}"
              class="btn btn-custom danger px-4 py-2 d-flex align-items-center justify-content-center ml-2"><i
                class="fas fa-times mr-2"></i>Reject Idea</a>
            <form action="{{ route('admin.admin-delete-idea', ['id' => $idea->id]) }}" method="POST">
              @csrf
              @method('DELETE')
              <button type="submit"
                class="btn btn-custom danger px-4 py-2 d-flex align-items-center justify-content-center ml-2"><i class="fas fa-trash mr-2"></i> Delete
                Idea</button>
            </form>
            @elseif($idea->verification_status == 'rejected')
            <a href="#" class="btn btn-custom danger px-4 py-2 d-flex align-items-center justify-content-center bg-danger">
              Idea Rejected</a>
            <form action="{{ route('admin.admin-delete-idea', ['id' => $idea->id]) }}" method="POST">
              @csrf
              @method('DELETE')
              <button type="submit"
                class="btn btn-custom danger px-4 py-2 d-flex align-items-center justify-content-center ml-2">Delete
                Idea</button>
            </form>
            @elseif($idea->verification_status == 'accepted')
            <a href="#" class="btn btn-custom success px-4 py-2 d-flex align-items-center justify-content-center bg-success">
              Idea Accepted</a>
            <form action="{{ route('admin.admin-delete-idea', ['id' => $idea->id]) }}" method="POST">
              @csrf
              @method('DELETE')
        ...
        <div class="switcher my-4">
          <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item">
              <a class="nav-link active" id="idea-detail" data-toggle="tab" href="#home" role="tab" aria-controls="home"
